...
--   public.user_training_enrollments
...
--   public.source_files
--   public.source_file_versions
--   public.source_sections
--   public.module_source_bindings
--
...
begin
  ...
  if not exists (
    ...
    alter table public.module_source_bindings
...
update public.source_file_versions v
set is_current = true
...
create unique index if not exists source_file_versions_one_current_per_source_idx
  on public.source_file_versions (source_file_id)
  where is_current;
...
-- -----------------------------------------------------------------------------
create table if not exists public.source_binders (
...
create table if not exists public.module_versions (
  id uuid primary key default gen_random_uuid(),
  -- FK intentionally omitted for now. The local baseline has public.training_modules,
  ...
  source_binder_version uuid references public.source_binders(id),
...
begin
  if not exists (
    ...
    alter table public.module_source_bindings
      ...
      foreign key (module_version_id) references public.module_versions(id) on delete cascade;
  end if;
end;
$$;
...
create table if not exists public.assessments (
  ...
  module_version_id uuid references public.module_versions(id) on delete cascade,
  title text not null,
...
-- -----------------------------------------------------------------------------
create table if not exists public.generated_content_reviews (
  id uuid primary key default gen_random_uuid(),
  module_version_id uuid not null references public.module_versions(id) on delete cascade,
  lesson_id uuid references public.training_lessons(id),
...
create index if not exists idx_assessment_attempts_lesson_id_attempted_at
  on public.assessment_attempts (lesson_id, attempted_at desc);
...
create index if not exists idx_source_binders_org_id_version_number
  on public.source_binders (org_id, version_number);
...
create index if not exists idx_acknowledgments_enrollment_id
  on public.acknowledgments (enrollment_id);

create index if not exists idx_generated_content_reviews_module_version_id
  on public.generated_content_reviews (module_version_id);
...
begin
  if not exists (
    ...
    execute 'create policy audit_logs_authenticated_read on public.audit_logs for select to authenticated using (true)';
  end if;
end;
$$;
